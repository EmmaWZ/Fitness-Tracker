...
function Petals() {
  const petals = Array.from({ length: 14 }, (_, i) => ({
    ...
    scale: 0.6 + Math.random() * 0.8, opacity: [0.55,0.4,0.65,0.35,0.5][i % 5],
  }));
  return (
    <div style={{ position:"fixed",inset:0,pointerEvents:"none",zIndex:1,overflow:"hidden" }}>
      <style>{`@keyframes petalFall{0%{transform:translateY(-30px) rotate(0deg) translateX(0);opacity:.7}40%{transform:translateY(40vh) rotate(120deg) translateX(18px);opacity:.5}80%{transform:translateY(80vh) rotate(260deg) translateX(-12px);opacity:.25}100%{transform:translateY(105vh) rotate(360deg) translateX(8px);opacity:0}}`}</style>
      {petals.map(p => (
        <div key={p.id} style={{ position:"absolute",left:p.left,top:0,animation:`petalFall ${p.dur} ${p.delay} infinite linear`,transform:`scale(${p.scale})` }}>
          <svg width="18" height="20" viewBox="0 0 18 20"><path d="M9 1 C12 4,16 8,14 13 C12 17,6 18,3 15 C0 12,1 6,4 3 C6 1,8 0,9 1Z" fill={`rgba(255,182,213,${p.opacity})`}/></svg>
        </div>
      ))}
    </div>
  );
...
export default function FitnessTracker() {
  ...
  const [defaultTasks, setDefaultTasks] = useState(() => {
    try { const s = localStorage.getItem("fit_default_tasks"); return s ? JSON.parse(s) : DEFAULT_TASKS; } catch { return DEFAULT_TASKS; }
  });
  // Extra tasks keyed by date: { "2025-01-01": [{id,icon,title,...}] }
  const [extraTasksByDay, setExtraTasksByDay] = useState(() => {
    try { const s = localStorage.getItem("fit_extra_tasks"); return s ? JSON.parse(s) : {}; } catch { return {}; }
  });
  // Checked state keyed by date: { "2025-01-01": { taskId: bool } }
  const [checkedByDay, setCheckedByDay] = useState(() => {
    try { const s = localStorage.getItem("fit_checked"); return s ? JSON.parse(s) : {}; } catch { return {}; }
  });
  const [goal, setGoal] = useState(() => {
    try { const s = localStorage.getItem("fit_goal"); return s ? JSON.parse(s) : DEFAULT_GOAL; } catch { return DEFAULT_GOAL; }
  });
  const [weeklyPlan, setWeeklyPlan] = useState(() => {
    try { const s = localStorage.getItem("fit_weekly_plan"); return s ? JSON.parse(s) : DEFAULT_WEEKLY_PLAN; } catch { return DEFAULT_WEEKLY_PLAN; }
  });
  ...
  // Weekly stats — total tasks per day = defaultTasks.length + that day's extras
  const weekDates = getWeekDates(weekKey);
  const weekStats = weekDates.map(dateStr => {
    try {
      const dayChecked = checkedByDay[dateStr];
      if (!dayChecked) return null;
      const extras = (extraTasksByDay[dateStr] || []).length;
      const total  = defaultTasks.length + extras;
  ...
  const toggle = (id) => {
    setCheckedByDay(p => ({
      ...p,
      [todayKey]: { ...(p[todayKey] || {}), [id]: !(p[todayKey]?.[id]) },
    }));
  };

  // Default task editing
  const [editingDefault, setEditingDefault] = useState(null);
  const [defaultDraft, setDefaultDraft]     = useState(null);
  const startEditDefault = t => { setDefaultDraft({ ...t }); setEditingDefault(t.id); setExpanded(null); };
  const saveDefault  = () => { setDefaultTasks(ts => ts.map(t => t.id === editingDefault ? { ...defaultDraft } : t)); setEditingDefault(null); };
  const deleteDefault = id => { setDefaultTasks(ts => ts.filter(t => t.id !== id)); setEditingDefault(null); };

  // Extra task adding / editing for today
  const [addingExtra, setAddingExtra]   = useState(false);
  const [newExtra, setNewExtra]         = useState({ icon:"✨", title:"", category:"今日专属", description:"", duration:"", tip:"", xp:15 });
  const [editingExtra, setEditingExtra] = useState(null);
  const [extraDraft, setExtraDraft]     = useState(null);

  const saveNewExtra = () => {
    if (!newExtra.title.trim()) return;
    const task = { ...newExtra, id: genId() };
    setExtraTasksByDay(p => ({ ...p, [todayKey]: [...(p[todayKey] || []), task] }));
    setNewExtra({ icon:"✨", title:"", category:"今日专属", description:"", duration:"", tip:"", xp:15 });
    setAddingExtra(false);
  };
  const startEditExtra = t => { setExtraDraft({ ...t }); setEditingExtra(t.id); setExpanded(null); };
  const saveExtra = () => {
    setExtraTasksByDay(p => ({ ...p, [todayKey]: (p[todayKey]||[]).map(t => t.id===editingExtra ? { ...extraDraft } : t) }));
    setEditingExtra(null);
  };
  const deleteExtra = id => {
    setExtraTasksByDay(p => ({ ...p, [todayKey]: (p[todayKey]||[]).filter(t => t.id!==id) }));
    setEditingExtra(null);
    setCheckedByDay(p => { const d = { ...(p[todayKey]||{}) }; delete d[id]; return { ...p, [todayKey]: d }; });
  };
  ...
  // Weekly plan editing
  const [editingWeekDay, setEditingWeekDay] = useState(null); // index 0-6
  const [weekDayDraft, setWeekDayDraft]     = useState(null);
  const startEditWeekDay = (i) => { setWeekDayDraft({ ...weeklyPlan[i] }); setEditingWeekDay(i); };
  const saveWeekDay = () => { setWeeklyPlan(p => p.map((d, i) => i===editingWeekDay ? { ...weekDayDraft } : d)); setEditingWeekDay(null); };
  ...
  const TaskRow = ({ task, isExtra = false }) => {
    const done = !!todayChecked[task.id];
    const isEditingThis = isExtra ? editingExtra===task.id : editingDefault===task.id;
    const draft = isExtra ? extraDraft : defaultDraft;
    const setDraft = isExtra ? setExtraDraft : setDefaultDraft;

    if (isEditingThis) {
      return (
        <div className="edit-form">
          <div style={{ display:"flex",gap:8,marginBottom:8 }}>
            <input className="finput" value={draft.icon} onChange={e=>setDraft(p=>({...p,icon:e.target.value}))} style={{ width:52,textAlign:"center",fontSize:20,flexShrink:0 }}/>
            <input className="finput" value={draft.title} onChange={e=>setDraft(p=>({...p,title:e.target.value}))} placeholder="任务名称"/>
          </div>
          <label className="flbl">分类</label>
          <input className="finput" value={draft.category} onChange={e=>setDraft(p=>({...p,category:e.target.value}))}/>
          <label className="flbl">描述</label>
          <input className="finput" value={draft.description} onChange={e=>setDraft(p=>({...p,description:e.target.value}))}/>
          <label className="flbl">时长/目标</label>
          <input className="finput" value={draft.duration} onChange={e=>setDraft(p=>({...p,duration:e.target.value}))}/>
          <label className="flbl">科学提示</label>
          <input className="finput" value={draft.tip} onChange={e=>setDraft(p=>({...p,tip:e.target.value}))}/>
          <label className="flbl">经验值 XP</label>
          <input className="finput" type="number" value={draft.xp} onChange={e=>setDraft(p=>({...p,xp:Number(e.target.value)}))}/>
          <div style={{ display:"flex",gap:8,marginTop:14 }}>
            <button className="save-btn" onClick={isExtra ? saveExtra : saveDefault}>保存</button>
            <button className="can-btn" onClick={()=>{ isExtra ? setEditingExtra(null) : setEditingDefault(null); }}>取消</button>
            <button className="del-btn" onClick={()=>{ isExtra ? deleteExtra(task.id) : deleteDefault(task.id); }} style={{ marginLeft:"auto" }}>删除</button>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className={`task-row ${done?"done":""}`} onClick={()=>setExpanded(expanded===task.id?null:task.id)}>
        <button className={`chk ${done?"done":""}`} onClick={e=>{e.stopPropagation();toggle(task.id);}}>{done?"✓":""}</button>
        <div style={{ fontSize:22,flexShrink:0 }}>{task.icon}</div>
        <div style={{ flex:1,minWidth:0 }}>
          <div style={{ marginBottom:3,display:"flex",alignItems:"center",gap:4,flexWrap:"wrap" }}>
            <span className="tag">{task.category}</span>
            <span className="xpb">+{task.xp} XP</span>
            {isExtra && <span style={{ fontSize:9,fontWeight:900,padding:"1px 6px",borderRadius:6,background:"rgba(255,240,180,0.85)",color:"#a07000" }}>今日专属</span>}
          </div>
          <div style={{ fontSize:14,fontWeight:800,color:done?"#c09ac0":"#5a006a",textDecoration:done?"line-through":"none" }}>{task.title}</div>
          <div style={{ fontSize:11,color:"#c068a0",fontWeight:700 }}>{task.description}{task.duration?` · ${task.duration}`:""}</div>
        </div>
        <button className="edt-btn" onClick={e=>{e.stopPropagation(); isExtra ? startEditExtra(task) : startEditDefault(task);}}>编辑</button>
        {expanded===task.id && task.tip && (
          <div className="tip-box" style={{ width:"100%" }}>💡 {task.tip}</div>
        )}
      </div>
    );
  };
    ...
    <div style={{ minHeight:"100vh",fontFamily:"'Nunito',sans-serif",color:"#3a003a",position:"relative",overflow:"hidden",
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0}
        ...
        .add-extra-btn:hover{background:rgba(255,250,200,0.4);border-color:rgba(200,160,0,0.4)}
      ...
      <div style={{ maxWidth:480,margin:"0 auto",padding:"24px 16px 72px",position:"relative",zIndex:2 }}>
        ...
        <div style={{ ...glass,padding:"22px 20px 20px",marginBottom:14,position:"relative",overflow:"hidden" }} className="fu">
          ...
          {editingGoal ? (
            <div>
              ...
              <input className="finput" value={goalDraft.subtitle} onChange={e=>setGoalDraft(p=>({...p,subtitle:e.target.value}))}/>
            ...
            <>
              ...
              <button className="edt-btn" onClick={()=>{ setGoalDraft(goal); setEditingGoal(true); }} style={{ position:"absolute",bottom:14,right:14,zIndex:2,background:"rgba(255,255,255,0.75)",border:"1px solid rgba(220,160,200,0.5)" }}>
                编辑目标
              </button>
            </>
          )}
        ...
        {tab==="today" && (
          <div className="fu">
            ...
            {defaultTasks.map(task => <TaskRow key={task.id} task={task} isExtra={false}/>)}
            ...
            {todayExtras.length === 0 && !addingExtra && (
              <div style={{ textAlign:"center",padding:"16px 0 8px",color:"#d0a0c8",fontSize:13,fontWeight:700 }}>
                今天还没有专属任务，加一个吧 ✨
              </div>
            )}

            {todayExtras.map(task => <TaskRow key={task.id} task={task} isExtra={true}/>)}

            {addingExtra ? (
              <div className="edit-form">
                <div style={{ fontSize:13,fontWeight:900,color:"#c09000",marginBottom:10 }}>✨ 添加今日专属任务</div>
                <div style={{ display:"flex",gap:8,marginBottom:8 }}>
                  <input className="finput" value={newExtra.icon} onChange={e=>setNewExtra(p=>({...p,icon:e.target.value}))} style={{ width:52,textAlign:"center",fontSize:20,flexShrink:0 }}/>
                  <input className="finput" value={newExtra.title} onChange={e=>setNewExtra(p=>({...p,title:e.target.value}))} placeholder="任务名称 *"/>
                </div>
                <label className="flbl">分类</label>
                <input className="finput" value={newExtra.category} onChange={e=>setNewExtra(p=>({...p,category:e.target.value}))} placeholder="例如：今日专属"/>
                <label className="flbl">描述</label>
                <input className="finput" value={newExtra.description} onChange={e=>setNewExtra(p=>({...p,description:e.target.value}))} placeholder="具体内容"/>
                <label className="flbl">时长/目标</label>
                <input className="finput" value={newExtra.duration} onChange={e=>setNewExtra(p=>({...p,duration:e.target.value}))} placeholder="例如：20 分钟"/>
                <label className="flbl">提示（可选）</label>
                <input className="finput" value={newExtra.tip} onChange={e=>setNewExtra(p=>({...p,tip:e.target.value}))} placeholder="小贴士"/>
                <label className="flbl">经验值 XP</label>
                <input className="finput" type="number" value={newExtra.xp} onChange={e=>setNewExtra(p=>({...p,xp:Number(e.target.value)}))}/>
                <div style={{ display:"flex",gap:8,marginTop:14 }}>
                  <button className="save-btn" onClick={saveNewExtra}>添加</button>
                  <button className="can-btn" onClick={()=>setAddingExtra(false)}>取消</button>
                </div>
              </div>
            ) : (
              ...
              if (editingWeekDay === i) {
                return (
                  <div key={i} className="edit-form">
                    ...
                    <input className="finput" value={weekDayDraft.emoji} onChange={e=>setWeekDayDraft(p=>({...p,emoji:e.target.value}))} style={{ fontSize:20,textAlign:"center" }}/>
                    <label className="flbl">训练内容描述</label>
                    <input className="finput" value={weekDayDraft.focus} onChange={e=>setWeekDayDraft(p=>({...p,focus:e.target.value}))} placeholder="例如：上肢力量 + 有氧"/>
                    <div style={{ display:"flex",gap:8,marginTop:14 }}>
                      <button className="save-btn" onClick={saveWeekDay}>保存</button>
                      <button className="can-btn" onClick={()=>setEditingWeekDay(null)}>取消</button>
                    </div>
                  </div>
                );
              }
                ...
                <div key={i} className={`wk-day ${isToday?"today-day":""} ${isFuture?"future-day":""}`}>
                  <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:10 }}>
                    ...
                    <div style={{ display:"flex",alignItems:"center",gap:6,flexShrink:0 }}>
                      ...
                      <button className="edt-btn" onClick={()=>startEditWeekDay(i)}>编辑</button>
                    </div>
                  </div>
